<template>
  <Dialog :visible="visible" :title="title" :loading="loading" @close="handleClose" @confirm="handleConfirm">
    <el-form ref="form" :model="form" :rules="rules" label-width="100px">
      ...
      <el-form-item v-if="form.authModeCode==='dictionary'" label="对应字典" prop="dictionaryCode">
        <drop-down-tree-selector
...
import { createField, updateField } from "@/api/authority/table";
...
export default {
  ...
  methods: {
    ...
    handleRequest(method, msg) {
      this.loading = true;
      this.form.tableId = this.currentRow.tableId;
      this[method](this.form).then(res => {
        if (res.statusCode !== 600) return;
        this.$emit("getFeildList");
        this.handleClose();
        this.msgSuccess(msg);
