<template>
  <div class="main-content-container">
    <list-container ref="ListContainer" api="/api/v1/rabbit/workflow/instance/find/page" :form-props="{'label-width': 'auto'}">
        ...
        <el-table v-loading="slot.loading" :data="slot.list" border @cell-dblclick="handleCellDblclick">
          ...
          <el-table-column label="操作" min-width="350" fixed="right">
            <template v-slot:default="slot">
              <el-row type="flex" :gutter="8">
                <el-col :span="1.5">
                  <Form :ref="`form${slot.row.id}`" :detail="slot.row" type="monitor" :button-method="buttonClick">
                ...
                <template v-for="(item, index) in getButtons()">
                  <el-col v-if="setCondition(item, slot.row)" :key="index" :span="1.5">
                    <map-jump v-if="item.type === 'jump' || item.type === 'retreat'" :id="slot.row.id" :detail="slot.row" :type="item.type">
                      <template v-slot:default="viewer">
                        <el-button type="text" @click="viewer.click">{{ item.name }}</el-button>
                      </template>
                    </map-jump>
                    <el-button v-else type="text" @click="buttonClick(item.type, slot.row)">{{ item.name }}</el-button>
                  </el-col>
                </template>
              </el-row>
            </template>
...
import { buttons, handleFunction, getTipMessage } from "./config";
import MapJump from "./component/map-jump";
...
export default {
  ...
  methods: {
    ...
    buttonClick(params, row) {
      const message = getTipMessage(params);
      this.$doingConfirm({ title: message.title, message: message.message }).then(() => {
        const payload = { id: row.id };
        handleFunction(params, payload).then(res => {
