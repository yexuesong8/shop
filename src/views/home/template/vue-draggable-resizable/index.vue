<template>
  ...
  >
    <div
      ...
      :style="handleStyle(actualHandle)"
...
export default {
  ...
  watch: {
    ...
    x(val) {
      ...
      if (this.parent) this.bounds = this.calcDragLimits();
  ...
  methods: {
    ...
    changeWidth(val) {
      ...
      console.log(this.parentWidth, this.left, right);
    ...
    formatTransformVal(string) {
      const value = string.replace(/[^0-9\-,]/g, '').split(',');
      const left = value[0];
      let top = value[1];
