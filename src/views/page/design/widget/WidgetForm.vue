    ...
    >
      ...
      >
        <transition-group name="fade" tag="div" class="widget-view-form-list">
          <div v-for="(childItem, childIndex) in element.children" :key="`widget-form-${childIndex}`">
...
export default {
  ...
  watch: {
    element: {
      handler(value) {
        const { props } = value;
        if (props && (props["page-form-padding"] || props["page-form-margin"])) {
          if (props["page-form-padding"]) {
            const padding = handleStringFunction(props["page-form-padding"]);
            this.$nextTick(() => {
              Object.keys(padding).forEach(item => {
                this.$refs["widgetForm"].style[item] = padding[item] + "px";
              });
            });
          }
          if (props["page-form-margin"]) {
            const margin = handleStringFunction(props["page-form-margin"]);
            this.$nextTick(() => {
              Object.keys(margin).forEach(item => {
                this.$refs["widgetForm"].style[item] = margin[item] + "px";
              });
            });
          }
        }
  ...
  methods: {
    ...
    handleWidgetAdd(event, element) {
      ...
      const { id, code, property, props = {}} = element.children[newIndex];
