    ...
    >
      ...
      >
        <transition-group name="fade" tag="div" class="widget-view-form-list">
          <div v-for="(childItem, childIndex) in element.children" :key="`widget-dialog-${childIndex}`">
            <template v-if="childItem.code === 'pg-dialog'">
              <widget-dialog
...
export default {
          ...
          if (props["page-container-padding"]) {
            ...
            this.$nextTick(() => {
              Object.keys(padding).forEach(item => {
                this.$refs["widgetDialog"].style[item] = padding[item] + "px";
              });
            });
          }
          if (props["page-container-margin"]) {
            ...
            this.$nextTick(() => {
              Object.keys(margin).forEach(item => {
                this.$refs["widgetDialog"].style[item] = margin[item] + "px";
              });
            });
          }
        }
  ...
  methods: {
    ...
    handleWidgetAdd(event, element) {
      ...
      if (code === 'pg-form' || code === 'pg-container') {
        const { children } = element.children[newIndex];
